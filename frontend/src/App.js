...
const API = "https://imagefeaturewebapp.herokuapp.com";
...
class App extends Component {
  ...
  getBase64Image() {
    return new Promise((resolve, reject) => {
      ...
      this.returnDim(imgElem.src).then((dim) => {
        ...
        var formattedData = dataURL.replace(
  ...
  postApi() {
    ...
    this.getBase64Image().then((data) => {
      const allData = Object.assign({}, this.getFormInfo(), { imgData: data });
      fetch(`${API}/api/v1/featureprocessing`, {
        method: "POST",
        headers: {
        ...
        <div className="contentContainer">
          ...
          <div className="controlsContainer">
            ...
            <div className="settingsContainer">
              ...
              <InputRange
                label="theta"
                value={this.state.lineTheta}
                callBack={this.onUpdate}
                min="0"
                max="3.14"
                step="0.017444444444444446"
              />
              <InputRange
                label="threshold"
                value={this.state.lineThreshold}
                callBack={this.onUpdate}
                min="0"
                max="300"
                step="1"
              />
              <InputRange
                label="line minLength"
                value={this.state.lineMinLength}
                callBack={this.onUpdate}
                min="0"
                max="1000"
                step="1"
            ...
            <div className="settingsContainer">
              <InputRange
                label="block size"
                value={this.state.cornerBlockSize}
                callBack={this.onUpdate}
                min="0"
                max="100"
                step="1"
            ...
            <br />
            <Button id="submitButton" onClick={this.postApi}>
              {" "}
              Send to API{" "}
            </Button>
          </div>
